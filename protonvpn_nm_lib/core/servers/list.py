import weakref, random, time, json
...
class LogicalServer:
    ...
    Due to the way Python works, data is basically a reference to the inside of the ServerList data.
    Beware that this is intended to be a short-lived object, if ServerList reloads completely, a 
    LogicalServer will not retain its bound to the list.
    """
    def __init__(self, data):
        self._data = data
...
class ServerList:
    ...
    All of these classes refer have an _ids property, which is the list of toplevel indices (logicals)
    this class has access to.
    
    When the toplevel
    """
    def __init__(self, toplevel = None, condition = None, sort_key = None, sort_reverse=False):
    ...
    def ensure_toplevel(self):
        if not self.is_toplevel:
            raise ValueError("This function has to be called in a toplevel ServerList")
    ...
    def __repr__(self):
        if self.is_toplevel:
            ...
            return 'ServerList<{}/{} servers>'.format(len(self), len(self._toplevel))
            
    def filter(self, condition):
        if self.is_toplevel:
            return ServerList(self, condition)
        else:
            return ServerList(self._toplevel, lambda x: self._condition(x) and condition(x))
    ...
    def get_fastest_server(self):
        ...
        servers_ordered = list(self.filter(lambda x: x.enabled).sort(lambda x: x.score))
        if len(servers_ordered) == 0:
            logger.error("List of logical servers is empty")
            raise exceptions.EmptyServerListError("No logical server could be found")            
    ...
    def _sort(self):
        ...
        if self._sort_key is None:
            ...
            self._ids.sort(key=lambda i: self._sort_key(LogicalServer(self._data['LogicalServers'][i])), reverse=self._sort_reverse)

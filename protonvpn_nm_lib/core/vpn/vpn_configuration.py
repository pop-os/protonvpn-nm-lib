...
class VPNConfiguration(SubclassesMixin, metaclass=ABCMeta):
    ...
    def __enter__(self):
        #We create the configuration file when we enter, and delete it when we exit.
        #This is a race free way of having temporary files.
        if self._configfile is None:
            self._configfile = tempfile.NamedTemporaryFile(dir=PROTON_XDG_CACHE_HOME, delete=False, prefix='ProtonVPN-', suffix=self.config_extn, mode='w')

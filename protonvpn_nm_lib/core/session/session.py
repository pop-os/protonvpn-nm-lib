...
from ...constants import (APP_VERSION, CACHED_SERVERLIST)
from ...enums import KeyringEnum
from ..environment import ExecutionEnvironment
from ...exceptions import JSONDataError, APISessionIsNotValidError
    ...
    def __call__(self, session, *args, **kwargs): 
        ...
        try:            
            ...
            if hasattr(self, f'_handle_{e.code}'):
                return getattr(self, f'_handle_{e.code}')(e, session, *args, **kwargs)
            else:
...
class APISession:
    ...
    - There's no valid reason why 1) would be missing but not 2), so we don't bother with logout in that case
    - 3) missing, but 1) and 2) are valid => fetch it from API.
    - 3) present, but 1) and 2) are missing => use it, but beware that API calls will fail (we could connect to VPN using cached data though)
    
    """
    
    #Probably would be better to have that somewhere else
    FULL_CACHE_TIME_EXPIRE = 180 * 60 #15min in seconds
    ...
    def __session_create(self):
        ...
        self.__proton_api = Session(self._api_url, 
    ...
    def __keyring_load_session(self):
        ...
        - if api_url doesn't match, just don't load the session (as it's for a different API)
        """
        try:
            keyring_data_user = ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_PROTON_USER.value]
        except KeyError:
        ...
        try:
            keyring_data = ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_SESSIONDATA.value]
        except KeyError:
        ...
        #Only now that we know everything is working, we will set info in the class
        from proton.api import Session
        #This is a "dangerous" call, as we assume that everything in keyring_data is correctly formatted
        self.__proton_api = Session.load(keyring_data, TLSPinning=self._enforce_pinning)
        self.__proton_user = keyring_data_user['proton_username']
        
    def __keyring_clear_session(self):
        for k in [KeyringEnum.DEFAULT_KEYRING_SESSIONDATA, KeyringEnum.DEFAULT_KEYRING_PROTON_USER]:
            try:
    ...
    def __keyring_clear_vpn_data(self):
        try:
            del ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_USERDATA.value]
        except KeyError:
            pass
    ...
    def refresh(self):
        ...
        ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_SESSIONDATA.value] = self.__proton_api.dump()
    ...
    def authenticate(self, username, password):
        ...
        #Order is important here: we first want to set keyrings, then set the class status to avoid inconstistencies
        ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_SESSIONDATA.value] = self.__proton_api.dump()
        ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_PROTON_USER.value] = {'proton_username': username}
    ...
    def is_valid(self):
        ...
        It doesn't check however if the session is working on the API though, so an API call might still fail.
        """
        #We use __proton_user as a proxy, since it's defined if and only if we have a logged-in session in __proton_api
    ...
    def __vpn_data_fetch_from_api(self):
        ...
        ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_USERDATA.value] = self.__vpn_data        
        
    @property
    def _vpn_data(self):
        """Get the vpn information.
        
        This is protected: we don't want anybody trying to mess with the JSON directly
        """
        #We have a local cache
        if self.__vpn_data is None:
            try:
                self.__vpn_data = ExecutionEnvironment().keyring[KeyringEnum.DEFAULT_KEYRING_USERDATA.value]
            except KeyError:
                #We couldn't load it from the keyring, but that's really not something exceptional.
    ...
    def _update_next_fetch_logicals(self):
        self.__next_fetch_logicals = self.__vpn_logicals.logicals_update_timestamp + \
            self.FULL_CACHE_TIME_EXPIRE * self.__generate_random_component()
    
    def _update_next_fetch_loads(self):
        self.__next_fetch_load = self.__vpn_logicals.logicals_update_timestamp + \
    ...
    def update_servers_if_needed(self, force=False):
        ...
        if changed:
            ...
            try:
                ...
                #This is not fatal, we only were not capable of storing the cache.
                logger.info("Could not save server cache")                
    ...
    def servers(self):
        if self.__vpn_logicals is None:
            ...
            try:
                ...
                #This is not fatal, we only were not capable of loading the cache.
    ...
    def update_client_config_if_needed(self, force=False):
        #FIXME: implement cache and replace this logic
        if self.__clientconfig is None or force:
            self.__clientconfig = self.__proton_api.api_request('/vpn/clientconfig')
